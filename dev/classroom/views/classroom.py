...
from ..forms import ProcessForm,PartnerAddForm, Partenaire_add,InputForm_add,OutputForm_add,TechForm_add,DecForm_add,SkillForm_add, EquipeForm,ActorForm_add, TaskForm, ActivityForm, TaskAddForm, ActivityAddForm, Type_actorForm, ActorForm, SkillForm, DecisionForm,  FinalProductForm, InputForm
from ..models import Process,Partner, Session, Activity,Task,  Actor, Skill, Decision,Type_actors, Input, Output, Technologie
...
class part_view_add(CreateView):
    model = Session
    form_class = Partenaire_add
    template_name = 'Home/Part/partenaire.html'
    ...
    def form_valid(self, form):
        ...
        quiz.save()
        form.save_m2m()
        return redirect('data_base', quiz.pk)
...
class PartCreateView(CreateView):
    model = Partner
    form_class = PartnerAddForm
    template_name = 'Home/Part/add.html'
...
class Table(ListView):
    ...
    ordering=('process_name')
    context_object_name='processes'
    template_name= 'Home/Table/table.html'
...
class ActivBaseView(ListView):
    model = Activity
    ordering=('activety_name')
    context_object_name='acti'
    template_name= 'Home/DataBase/activ.html'


    def get_queryset(self):
        process = get_object_or_404(Process, pk=self.kwargs['pk'])
        queryset = Activity.objects.filter(process_id=process)
        return queryset

    def get_context_data(self, **kwargs):
        context = super(ActivBaseView, self).get_context_data(**kwargs)
        context['proc'] = get_object_or_404(Process, pk=self.kwargs['pk'])
        return context
...
class TaskBaseView(ListView):
    ...
    template_name= 'Home/DataBase/task.html'


    def get_queryset(self):
        acti = get_object_or_404(Activity, pk=self.kwargs['pk'])
        queryset = Task.objects.filter(activety_id=acti)
        return queryset

    def get_context_data(self, **kwargs):
        context = super(TaskBaseView, self).get_context_data(**kwargs)
        context['proc'] = get_object_or_404(Activity, pk=self.kwargs['pk'])
        return context
...
class Actor_change_view(UpdateView):
    ...
    def form_valid(self, form):
        task = get_object_or_404(Task, pk=self.kwargs['pk_1'])
        quiz = form.save(commit=False)
        quiz.save()
        form.save_m2m()
        return redirect('actor_change', quiz.pk, task.pk)
    

    def get_context_data(self, **kwargs):
        # Call the base implementation first to get a context
        context = super(Actor_change_view, self).get_context_data(**kwargs)
        context['actor'] = get_object_or_404(Actor, pk=self.kwargs['pk'])
        context['task'] = get_object_or_404(Task, pk=self.kwargs['pk_1'])
        return context
...
class ActorDeleteView(DeleteView):
    ...
    def get_context_data(self, **kwargs):
        # Call the base implementation first to get a context
        context = super(ActorDeleteView, self).get_context_data(**kwargs)
        context['actor'] = get_object_or_404(Actor, pk=self.kwargs['pk'])
        context['task'] = get_object_or_404(Task, pk=self.kwargs['pk_1'])
        return context
...
class input_change_add(UpdateView):
    ...
    def form_valid(self, form):
        ...
        quiz.save()
        form.save_m2m()
        return redirect('input_change', quiz.pk , task.pk)

    def get_context_data(self, **kwargs):
        # Call the base implementation first to get a context
        context = super(input_change_add, self).get_context_data(**kwargs)
        context['input'] = get_object_or_404(Input, pk=self.kwargs['pk'])
        context['task'] = get_object_or_404(Task, pk=self.kwargs['pk_1'])
        return context
...
class InputDeleteView(DeleteView):
    ...
    def get_context_data(self, **kwargs):
        # Call the base implementation first to get a context
        context = super(InputDeleteView, self).get_context_data(**kwargs)
        context['input'] = get_object_or_404(Input, pk=self.kwargs['pk'])
        context['task'] = get_object_or_404(Task, pk=self.kwargs['pk_1'])
        return context
...
class Output_change(UpdateView):
    ...
    def form_valid(self, form):
        ...
        quiz.save()
        form.save_m2m()
        return redirect('output_change', quiz.pk, act.pk)
    
    def get_context_data(self, **kwargs):
        # Call the base implementation first to get a context
        context = super(Output_change, self).get_context_data(**kwargs)
        context['output'] = get_object_or_404(Output, pk=self.kwargs['pk'])
        context['task'] = get_object_or_404(Task, pk=self.kwargs['pk_1'])
        return context    
...
class OutputDeleteView(DeleteView):
    ...
    def get_context_data(self, **kwargs):
        # Call the base implementation first to get a context
        context = super(OutputDeleteView, self).get_context_data(**kwargs)
        context['output'] = get_object_or_404(Output, pk=self.kwargs['pk'])
        context['task'] = get_object_or_404(Task, pk=self.kwargs['pk_1'])
        return context
...
class tech_change(UpdateView):
    ...
    def form_valid(self, form):
        ...
        quiz.save()
        form.save_m2m()
        return redirect('tech_change', quiz.pk, act.pk)
    
    def get_context_data(self, **kwargs):
        # Call the base implementation first to get a context
        context = super(tech_change, self).get_context_data(**kwargs)
        context['tech'] = get_object_or_404(Technologie, pk=self.kwargs['pk'])
        context['task'] = get_object_or_404(Task, pk=self.kwargs['pk_1'])
        return context    
...
class techDeleteView(DeleteView):
    ...
    def get_context_data(self, **kwargs):
        # Call the base implementation first to get a context
        context = super(techDeleteView, self).get_context_data(**kwargs)
        context['tech'] = get_object_or_404(Technologie, pk=self.kwargs['pk'])
        context['task'] = get_object_or_404(Task, pk=self.kwargs['pk_1'])
        return context
...
class skill_change(UpdateView):
    ...
    def form_valid(self, form):
        ...
        quiz.save()
        form.save_m2m()
        return redirect('skill_change', quiz.pk, act.pk)
    
    def get_context_data(self, **kwargs):
        # Call the base implementation first to get a context
        context = super(skill_change, self).get_context_data(**kwargs)
...
class dec_change(UpdateView):
    ...
    def form_valid(self, form):
        ...
        quiz.save()
        form.save_m2m()
        return redirect('dec_change', quiz.pk, act.pk)
    
    def get_context_data(self, **kwargs):
        # Call the base implementation first to get a context
        context = super(dec_change, self).get_context_data(**kwargs)
        context['dec'] = get_object_or_404(Decision, pk=self.kwargs['pk'])
        context['task'] = get_object_or_404(Task, pk=self.kwargs['pk_1'])
        return context    
...
class decDeleteView(DeleteView):
    ...
    def get_context_data(self, **kwargs):
        # Call the base implementation first to get a context
        context = super(decDeleteView, self).get_context_data(**kwargs)
        context['dec'] = get_object_or_404(Decision, pk=self.kwargs['pk'])
        context['task'] = get_object_or_404(Task, pk=self.kwargs['pk_1'])
        return context        
...
class TaskDetaiListView(ListView):
    model = Task
    template_name = 'Home/DataBase/task_display.html'
...
class add_actor_element(CreateView):
    ...
    def form_valid(self, form):
        ...
        case.save()
        form.save_m2m()
        return redirect('actor_change', case.pk , act.pk)

    def get_context_data(self, **kwargs):
        context = super(add_actor_element, self).get_context_data(**kwargs)
        context['task'] = get_object_or_404(Task, pk=self.kwargs['pk'])
        return context
...
class add_input_element(CreateView):
    model = Input
    fields = ['input_name', 'definition', 'part']
    ...
    def form_valid(self, form):
        ...
        case.save()
        form.save_m2m()
        return redirect('input_change', case.pk, act.pk)    

    def get_context_data(self, **kwargs):
        context = super(add_input_element, self).get_context_data(**kwargs)
        context['task'] = get_object_or_404(Task, pk=self.kwargs['pk'])
        return context
...
class add_output_element(CreateView):
    model = Output
    fields = ['output_name', 'definition', 'part']
    ...
    def form_valid(self, form):
        ...
        case.task_id= act
        case.save()
        return redirect('output_change', case.pk, act.pk)
    


class add_dec_element(CreateView):
    model = Decision
    fields = ['decision_name', 'definition', 'part']
    ...
    def form_valid(self, form):
        ...
        case.task_id= act
        case.save()
        return redirect('dec_change', case.pk, act.pk)

    def get_context_data(self, **kwargs):
        context = super(add_dec_element, self).get_context_data(**kwargs)
        context['task'] = get_object_or_404(Task, pk=self.kwargs['pk'])
        return context
...
class add_tech_element(CreateView):
    model = Technologie
    fields = ['technologie_name', 'technology_maturity','definition', 'part']
    ...
    def form_valid(self, form):
        ...
        case.save()
        form.save_m2m()
        return redirect('tech_change', case.pk, act.pk)    

    def get_context_data(self, **kwargs):
        context = super(add_tech_element, self).get_context_data(**kwargs)
        context['task'] = get_object_or_404(Task, pk=self.kwargs['pk'])
        return context
...
class add_skill_element(CreateView):
    model = Skill
    fields = ['skills_name', 'skills_maturity', 'definition', 'part']
    ...
    def form_valid(self, form):
        ...
        case.save()
        form.save_m2m()
        return redirect('skill_change', case.pk, act.pk)    

    def get_context_data(self, **kwargs):
        context = super(add_skill_element, self).get_context_data(**kwargs)
        context['task'] = get_object_or_404(Task, pk=self.kwargs['pk'])
        return context
